...
PixelClassColourModel::PixelClassColourModel(ColourHistLUT** lut, int h_bins, int s_bins, int v_bins, int _niScales, int imgw, int imgh)
{
  mImageBB.initPosAndSize(0, 0, imgw, imgh);
  for(int i=0; i<3; i++)
  {
    mHist[i] = new Histogram(h_bins, s_bins, v_bins, _niScales, mImageBB, lut[0]->hasSeparate3rdChannel());
    mUpdateHist[i] = new Histogram(h_bins, s_bins, v_bins, _niScales, mImageBB, lut[0]->hasSeparate3rdChannel());
  }
  mLUT = lut;
  mfMeanFGVoteErr = -1;
}
...
void PixelClassColourModel::create(Image8U* img, Rectangle* outer_object, Rectangle* object_region, float enlarge_factor_for_bg/*=2.0*/)
{
  Rectangle outer_top, outer_bottom, outer_left, outer_right, margin;
  int hdiff, wdiff;
  margin = *outer_object;
  Rectangle outer = margin;
  outer.enlarge(enlarge_factor_for_bg);
...
void PixelClassColourModel::update(Image8U* img, Rectangle* outer_bb, Image<float>* segmentation, Image<float>* bp_img, float update_factor, float enlarge_factor_for_bg/*=2.0*/)
{
  Rectangle outer_top, outer_bottom, outer_left, outer_right, margin;
  int hdiff, wdiff;
  margin = *outer_bb;
