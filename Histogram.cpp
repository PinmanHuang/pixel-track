...
Histogram::Histogram(int h_bins, int s_bins, int v_bins, int _niScales, Rectangle imgBB, bool v_bin_separate/*=true*/)
{
  niScales = _niScales;
  ...
  miNTotalBins = new int[niScales];
  for(int s=0;s<niScales;s++){
    if (v_bin_separate)
      miNTotalBins[s] = h_bins*(s+1)*s_bins*(s+1)+v_bins*(s+1);
    else
      miNTotalBins[s] = h_bins*(s+1)*s_bins*(s+1)*v_bins*(s+1);
    hsv_count[s].resize(miNTotalBins[s]);
    mTmpres[s].resize(miNTotalBins[s]);
...
int Histogram::compute(Image<unsigned char>* img, ColourHistLUT** luts, Rectangle* roi, Image<float>* mask/*=NULL*/, bool set_zero/*=true*/, bool normalise_hist/*=true*/, bool grid/*=true*/){
  if(set_zero){
    setZero();
  }
  ...
  if(mask == NULL){  
	...
	for(i=0;i<roinx;i++){
	  ...
	  for(s=0;s<niScales;s++){
	    hsv_count[s][luts[s]->bgr_to_colour_bin[(((b<<8) + g)<<8) + r]]++;
	    //hsv_count[s][luts[s]->getBin(r, g, b)]++;
	  }
	  ptr+=pixelstep;
	}
	ptr+=xstep;
...
int Histogram::compute(Image<unsigned char>* img, ColourHistLUT** luts,
                Rectangle* roi, 
		...
		bool grid/*=true*/)
{
  ...
  int rw2=roi->miWidth/2, rh2=roi->miHeight/2;
	...
	for(i=0;i<roinx;i++){
	  ...
	  r = *(ptr++);
	  for(s=0;s<niScales;s++){
	    hsv_count[s][luts[s]->bgr_to_colour_bin[(((b<<8) + g)<<8) + r]]+=spatial_prior;
	    //hsv_count[s][luts[s]->getBin(r, g, s)]+=spatial_prior;
	  }
	  ptr+=pixelstep;
	}
	ptr+=xstep;
